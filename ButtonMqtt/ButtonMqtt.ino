...
char broker_url[] = "m23.cloudmqtt.com";
...
void setup() {
  ...
  Serial1.begin(9600);
  WiFi.init(&Serial1);
  client.setServer( broker_url, 14160 );
...
void loop() {
  ...
  // Check the wifi and mqtt connection every 3 second
  ...
  if(millis() - lastSend > 20){
    ...
    if (buttonState == HIGH && lastButtonState == LOW) {
      ...
      Serial.println("Publishing rooms/1/light: switch,arduinoClient");
      client.publish( "rooms/1/light", "switch,arduinoClient" );
    }
...
void connect_to_broker() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("arduinoClient", "roomUser", "roomUser")) {
